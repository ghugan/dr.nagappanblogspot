...
function oblique_custom_styles( $custom ) {
	...
	// Post inner
	$post_inner = get_theme_mod( 'post_inner_color', apply_filters( 'oblique_post_inner_color', '#ffffff' ) );
	if ( ! empty( $site_desc ) ) {
		$custom .= '.post-inner, .woocommerce-page ul.products li.product, .woocommerce-page ul.products li.product div.post-bottom-svg { background-color:' . esc_attr( $post_inner ) . '}' . "\n";
	}
